  ...
  module.exports={
    ...
    data() {
      return {
        ...
        cur_total:0,
        url:'',
        sendList:[{'typename': '编号'},{'typename': '姓名'},{'typename': '性别'}],
        pickerOptions1: {
          disabledDate(time) {
...
methods:{
  ...
  getData(){
    // 开发环境使用 easy-mock 数据，正式环境使用 json 文件
      this.url = '/selectAllWorker/'+this.cur_page+'/'+this.cur_page_size;
  ...
  getAllNum(){
    //获得公司员工的总人数
    axios.get('/selectWorkerNum').then((res) => {
  ...
  search() {
    ...
    search.append('selectCate',this.select_cate);
    search.append('selectWord',this.select_word);
    search.append('startTime',this.value1);
    search.append('endTime',this.value2);
